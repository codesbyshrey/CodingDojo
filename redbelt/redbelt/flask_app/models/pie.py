...
##### CLASS METHODS ARE HERE ########
     @classmethod
     def create(cls, data):
          query = "INSERT INTO recipes (name, filling, crust, user_pie_id) VALUES (%(name)s,%(filling)s,%(crust)s,%(user_pie_id)s);"
...
# Geting all pies (different from all pies for a user) - need to JOIN here like in dojos and ninjas so that we can properly display all the pies on our Derby page
     @classmethod
     def get_all(cls):
          # triple quotes makes it hard to parse some of this properly but make sit easier to read all of our query on one line, especially if its long like in this one
          # SQL countries comes in handy here tbh, learning about COUNT was kinda crucial and understanding how to do multi-join with long queries
          # remember to order by votes in a descending order
          query = " SELECT pies.id, pies.user_pie_id, pies.pie_name, users.first_name, COUNT(votes.id) AS votes FROM pies JOIN users ON pies.user_pie_id = users.id LEFT JOIN votes ON pies.id = votes.pie_id GROUP BY pies.id ORDER BY votes DESC;"

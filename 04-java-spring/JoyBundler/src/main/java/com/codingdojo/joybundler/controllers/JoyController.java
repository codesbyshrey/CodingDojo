...
public class JoyController {
	...
	public String showJoy(@PathVariable("joyID") Long joyID, Model model, HttpSession session) {
		...
		System.out.println("ARE WE RUNNING INTO AN ERROR BEFORE OR ATER THIS LINE");
	...
	@PutMapping("/names/updateJoy")
	public String updateJoy(@Valid @ModelAttribute("joy") Joy joy, BindingResult result, HttpSession session) {
		// If User not logged in, redirect
		if (session.getAttribute("userID") == null) {
			return "redirect:/";
		}
